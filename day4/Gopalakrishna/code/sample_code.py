...
class nnpde_informed_B1():
    def __init__(self,linearTermE,advection_z_e,diffusion_z,J0_e,z,num_layers,batchSize,lr,adam_iter,dt,
        ...
        self.J0_e_t = torch.tensor(self.J0_e)
        self.J0_h_t = torch.tensor(self.J0_h)

    def NN(self,inputDim,num_layers,num_neurons=30):
        layers = [nn.Linear(inputDim,num_neurons),nn.Tanh()]
        ...
        for m in model_:
            if isinstance(m,nn.Linear):
                nn.init.xavier_normal_(m.weight) #Glorot normal initialization
                nn.init.zeros_(m.bias)
        return model_

    def get_value_pde(self,value_function_e,value_function_h,X_pde,idx,false_transient,agent_type):
        z = torch.tensor(X_pde[idx,0:1],requires_grad=True)
        t = torch.tensor(X_pde[idx,1:2],requires_grad=True)
        if agent_type=='households': u = value_function_h(torch.cat([z,t],dim=1))
        elif agent_type=='experts': u = value_function_e(torch.cat([z,t],dim=1))
        u_z = torch.autograd.grad(u,z,grad_outputs=torch.ones_like(u),create_graph=True)[0]
        u_t = torch.autograd.grad(u,t,grad_outputs=torch.ones_like(u),create_graph=True)[0]
        u_zz = torch.autograd.grad(u_z,z,grad_outputs=torch.ones_like(u_z),create_graph=True)[0]

        if agent_type =='experts':
            advection_z = torch.tensor(self.advection_z_e.transpose().reshape(-1,1)[idx])
            linearTerm = torch.tensor(self.linearTermE.transpose().reshape(-1,1)[idx])
        elif agent_type=='households':
            advection_z = torch.tensor(self.advection_z_h.transpose().reshape(-1,1)[idx])
            linearTerm = torch.tensor(self.linearTermH.transpose().reshape(-1,1)[idx])

        diffusion_z = torch.tensor(self.diffusion_z.transpose().reshape(-1,1)[idx])
    ...
    def PDESolver(self,value_function_e,value_function_h,j0_e,j0_h,X,X_pde,idx,idx_crisis):
        ...
        jE = value_function_e(self.X_t)

        fpde_h = self.get_value_pde(value_function_e,value_function_h,X_pde,idx,True,'households')
        jH = value_function_h(self.X_t)

        #experts
        loss_1_1 = fpde_e
        loss_1 = torch.mean(torch.square(loss_1_1))

        loss_2 = torch.mean(torch.square(jE - j0_e))

        #households
        loss_3 = torch.mean(torch.square(jH - j0_h))
        loss_4 = torch.mean(torch.square(fpde_h))
    ...
    def loss_function(self,batchSize):
        ...
        loss_total,fpde = self.PDESolver(self.value_function_e,self.value_function_h,self.J0_e_t,self.J0_h_t,self.X,self.X_pde,idx,idx)
    ...
    def train(self):
        ...
        if os.path.isfile('./save/value_function_experts.pt'):
            self.value_function_e.load_state_dict(torch.load('./save/value_function_experts.pt'))

        self.value_function_h = self.NN(2,self.params['num_layers'])
        if os.path.isfile('./save/value_function_households.pt'):
            self.value_function_h.load_state_dict(torch.load('./save/value_function_households.pt'))

        self.theta_valueFunction_e = list(self.value_function_e.parameters())
        self.theta_valueFunction_h = list(self.value_function_h.parameters())
        self.optimizer = torch.optim.Adam(self.theta_valueFunction_e + self.theta_valueFunction_h,lr=self.lr)

        self.best_valueFunction_e = copy.deepcopy(self.value_function_e.state_dict())
        self.best_valueFunction_h = copy.deepcopy(self.value_function_h.state_dict())
        ...
        for epoch in range(self.number_epochs+1):
                ...
                if epoch%1000==0: print('It: %d, Loss: %.3e, Time: %.2f'%
                          (epoch, min_loss, elapsed_time))
                self.LVF.append(loss_value)
        self.value_function_e.load_state_dict(self.best_valueFunction_e)
        self.value_function_h.load_state_dict(self.best_valueFunction_h)
        self.fpde = fpde.detach()

    def predict(self,x_star):
        with torch.no_grad():
            x = torch.tensor(x_star)
            je_new = self.value_function_e(x)
            jh_new = self.value_function_h(x)
    ...
    def solve(self,pde=True):
        ...
        for timeStep in range(self.params['maxIterations']):
            ...
            if pde==True:
                ...
                torch.save(model.value_function_e.state_dict(),'./save/value_function_experts.pt')
                torch.save(model.value_function_h.state_dict(),'./save/value_function_households.pt')
